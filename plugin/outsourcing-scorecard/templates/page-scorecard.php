 ...
 * Full standalone HTML document for the Outsourcing Readiness Scorecard.
 * Completely overrides the active WordPress theme — zero theme interference.
 *
 * ALL assets loaded from plugin/dist/ (built by npm run build):
 *   dist/js/vendor/jquery.min.js
 *   dist/js/vendor/bootstrap.bundle.min.js
 *   dist/js/vendor/intlTelInput.min.js
 *   dist/css/vendor/bootstrap.min.css
 *   dist/css/vendor/bootstrap-icons.min.css
 *   dist/css/vendor/intlTelInput.css
 *   dist/js/scorecard.js       ← IIFE with jsPDF bundled in
 *   dist/css/scorecard.css     ← quiz styles
 * No CDN dependencies — works on fully air-gapped servers.
 *
 * window.MagellanConfig is read by scorecard.js for all runtime config.
 */

if ( ! defined( 'ABSPATH' ) ) exit;

$dist      = OSC_DIST_URL;          // plugin/dist/
$assets    = OSC_ASSETS_URL;        // plugin/assets/
...
<head>
    ...
    <title>Outsourcing Readiness Scorecard &#8212; <?php bloginfo( 'name' ); ?></title>
    ...
    <!-- Vendor CSS — from npm, copied to dist/css/vendor/ by build.mjs — NO CDN -->
    ...
    <!-- Runtime config injected before any JS loads -->
    <script>
    window.MagellanConfig = {
        ...
        assetsBase:       <?php echo wp_json_encode( $assets ); ?>,
        distBase:         <?php echo wp_json_encode( $dist ); ?>
    };
    </script>
...
<body>
    ...
    <div id="landing" class="d-flex flex-column min-vh-100"
        ...
        <nav class="landing-nav d-flex align-items-center justify-content-between px-4 px-lg-5 py-4 position-relative" style="z-index:2;">
            <span class="nav-logo">
                <img src="<?php echo esc_url( $assets . 'logo.webp' ); ?>" alt="Magellan Solutions" width="300">
            </span>
        ...
        <div class="landing-grid flex-grow-1 row align-items-center g-0 px-4 px-lg-5 pb-5 position-relative mx-auto w-100"
             style="z-index:2; max-width:1280px;">
        ...
        <div id="quizWrapper" class="d-none flex-grow-1 d-flex align-items-center">
            ...
            <div class="quiz-body container py-5">
                <form id="quizForm" novalidate>
                    ...
                    <div class="d-flex justify-content-center mt-4">
                        <button type="submit" id="submitBtn"
                                class="cta-btn d-none d-inline-flex align-items-center gap-2">
                            Check Readiness
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                 stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M5 12h14M12 5l7 7-7 7"/>
                ...
                <div class="quiz-bottom-nav mt-4 d-flex align-items-center justify-content-center gap-3">
                    <button type="button" id="prevBtnMobile"
                            class="quiz-arrow quiz-arrow--inline" aria-label="Previous">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                             stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M15 18l-6-6 6-6"/>
                        </svg>
                    ...
                    <button type="button" id="nextBtnMobile"
                            class="quiz-arrow quiz-arrow--inline" aria-label="Next">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                             stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="trust-bar d-flex align-items-center justify-content-center gap-3 px-4 px-lg-5 py-3 border-top position-relative">
            ...
            <span>&copy; <?php echo gmdate( 'Y' ); ?></span>
        </div>
    ...
    <div id="popup" class="d-none">
        <button id="closePopup" type="button"
                class="float-end border-0 bg-transparent fs-4 lh-1">&times;</button>
    ...
    <!--
        Vendor JS — loaded from dist/vendor/ (npm packages, no CDN).
        ORDER MATTERS:
          1. jQuery        — must be first (scorecard.js wraps in jQuery IIFE)
          2. Bootstrap     — depends on jQuery already being present
          3. intlTelInput  — must be present before scorecard.js calls intlTelInput()
          4. scorecard.js  — IIFE with jsPDF prepended by build.mjs
    -->
    ...
    <!-- Plugin JS bundle (jsPDF prepended + scorecard IIFE, minified) -->
