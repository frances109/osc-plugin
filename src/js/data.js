/**
 * data.js
 * Single source of truth for all quiz content and scoring rules.
 * Pure data — no DOM, no side effects. Import anywhere safely.
 *
 * Exports:
 *   CONFIG         — clusters of questions rendered by form-builder.js
 *   SCORING_RULES  — which answers carry points and which insight message
 *   TIERS          — score thresholds, result copy, and CTA definitions
 */
...
// ── Scoring rules ─────────────────────────────────────────────────────────────
// Each rule maps one form field to points + insight message.
// The '_' key is the catch-all fallback when no explicit case matches.
...
// ── Readiness tiers ───────────────────────────────────────────────────────────
// Ordered highest-first. matchTier() in scoring.js picks the first where score >= min.
//
// CTA actions:
//   'schedule'  — sends admin email (Discovery Call)
//   'consult'   — sends admin email (Consultation)
//   'download'  — downloads the static readiness-guide.pdf
...
export const TIERS = [
  ...
  },
  {
    ...
    ctas:     [
      { label: 'Book a Consultation',        action: 'consult' },
      { label: 'Download Our Readiness Guide', action: 'download' },
    ],
  },
  {
    min:      0,
